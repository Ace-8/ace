...
import com.jfinal.AI.Baidu.Animal_PLANT;
import com.jfinal.AI.Baidu.utils.Utiles;
import com.jfinal.AI.Tencent.Conversation;
...
import com.jfinal.weixin.utils.SongUtils;
...
public class WeixinMsgController extends MsgControllerAdapter {
    ...
    protected void processInTextMsg(InTextMsg inTextMsg) {
        ...
        String playName = "";  // 存放用户的 搜索内容
        if(content.startsWith("音乐#")) {
        	playName = MovieUtils.splitPlayName(content);
        	if("".equals(playName)) {
            	outTextMsg.setContent("搜索内容不能为空~");
                render(outTextMsg);
            } else {
    	        String result = SongUtils.musicList(playName, 1);
    	        // result:{"success":false,"message":"CSRF token Invalid!","now":"2020-11-18T11:17:14.103Z"}
    	        System.out.println("result:" + result);
    	        
    	        // 判断 result 的值是否正常
    	        Boolean boolean1 = SongUtils.musicListWhetherTrue(result);
    	        System.out.println("boolean1: " + boolean1);
    	        
    	        // 如果 result 正常
    	        if(boolean1) {
    	        	String musicNameString = "";   // 实际返回的歌曲名
        	        String ridString = "";  // 实际返回的歌曲rid
        	        String artist = "";  // 歌手名
//        	        对 JSON 数据进行解析
...
//            	            musicNameString = music.getString("name");
//            	            ridString = music.getString("rid");
//            	            
//                	        // 根据 rid 获取歌曲链接
//                	        String musicUrl = Utils.get_url_by_rid(ridString);
//                	        System.out.println("musicUrl:" + musicUrl);
//                	        
////                	        outMusicMsg.setHqMusicUrl(musicUrl);
////                	        outMusicMsg.setTitle(musicNameString);
////                	        render(outMusicMsg);
//                	        reString += "<a href="+musicUrl+">"+musicNameString+"</a>" + "\n";
//                	        outTextMsg.setContent(reString);
//            	        }
            	        ...
            	        for (int i = 0; i < list.size(); i++) {
    						...
    						musicNameString = music.getString("name");
            	            ridString = music.getString("rid");
            	            artist = music.getString("artist");
            	            
                	        // 根据 rid 获取歌曲链接
            	            // TODO 搜索 李宇春: 存在 bug
                	        String musicUrl = SongUtils.get_url_by_rid(ridString);
                	        System.out.println("musicUrl:" + musicUrl);
                	        if(i==list.size()-1) {
                	        	reString += "<a href=" + musicUrl + ">" + "#" + musicNameString + "  [" + artist  +"]" + "</a>";
//                	        	reString += musicNameString;
                	        } else {
                	        	reString += "<a href=" + musicUrl + ">" + "#" + musicNameString + "  [" + artist  +"]" + "</a>" + "\n";
//                	        	reString += musicNameString + "\n";
                	        }
        	        	...
        	        	outTextMsg.setContent("没有这首歌呢~");
                        render(outTextMsg);
        	        }
    	        } else {
    	        	outTextMsg.setContent("您的网络有点开小差呢~");
                    render(outTextMsg);
    	        }
            }
        	...
        	if("".equals(playName)) {
            	...
            	System.out.println("playName_movie:" + playName);
            	
            	OutNewsMsg outNewsMsg = new OutNewsMsg(inTextMsg);
            	News news = new News();
            	news.setPicUrl(Property.MOVIEIMGURL);  // 设置显示的图片
            	news.setTitle(playName);
            	news.setUrl(Property.HTTPADDR + "/movie/index?searchName="+playName);
            	news.setDescription("" + "\n" + "\n" + playName);
            	outNewsMsg.addNews(news);
                render(outNewsMsg);
            }
		} else if(content.startsWith("帮助") || "help".equalsIgnoreCase(content)) {
			OutNewsMsg outNewsMsg = new OutNewsMsg(inTextMsg);
        	News news = new News();
        	news.setPicUrl(Property.HELPIMAGEURL);  // 设置显示的图片
        	news.setTitle("获取帮助");
        	news.setUrl(Property.HELPURL);
        	news.setDescription("获取帮助");
        	outNewsMsg.addNews(news);
            render(outNewsMsg);
		} else {
			// 智能对话
			String result = Conversation.ConversationCall(content);
			outTextMsg.setContent(result);
            render(outTextMsg);
		}
    }
    ...
    protected void processInImageMsg(InImageMsg inImageMsg)
    {
    	...
    	String imagePath = inImageMsg.getPicUrl();  // 图片路径 [本地|网络]
        ...
        try {
			...
			try {
				imagePath = Utiles.downloadPicture("F:\\img\\", imagePath);
				imgData = FileUtil.readFileByBytes(imagePath);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
    	...
    	String jsonResult = "";  // 识别的json数据
    	ArrayList<String> parseResult = new ArrayList<String>();  // 图片解析结果
    	
    	// 进行动物识别
    	jsonResult = Animal_PLANT.identify(imgData, imagePath, Property.ANIMAL_URL);
        parseResult = Animal_PLANT.parseJsonResult(jsonResult);
		
        // {"log_id": 4625472003150246291, "result": [{"score": "0.999986", "name": "非动物"}]}
        if("非动物".equals(parseResult.get(1))) {
        	// 非动物,则进行植物识别 (直接读取本地图片)
        	
        	// 进行植物识别
        	jsonResult = Animal_PLANT.identify(imgData, imagePath, Property.PLANT_URL);
            parseResult = Animal_PLANT.parseJsonResult(jsonResult);
            
//            System.out.println("##########################################");
//            System.out.println(parseResult.get(1));
//            System.out.println(!("".equals(parseResult.get(1))));

            if("非植物".equals(parseResult.get(1))) {
            	// 非动物 非植物, 则进行图片风格转换
        ...
        outTextMsg.setContent(parseResult.get(0));
        render(outTextMsg);
    }
    ...
    protected void processInVideoMsg(InVideoMsg inVideoMsg) {
      ...
      outMsg.setContent("视频消息~");
//      outMsg.setContent("视频消息~  该视频的 mediaId 为: " + inVideoMsg.getMediaId());
      render(outMsg);
    }
...
//            render(outMsg);
        	...
        	News news = new News();
        	news.setPicUrl(Property.HELPIMGURL);  // 设置显示的图片
        	news.setTitle("欢迎关注 csStudy 订阅号");
        	news.setUrl(Property.HELPURL);
        	news.setDescription("" + "\n" + "\n" + "使用帮助");
        	outNewsMsg.addNews(news);
            render(outNewsMsg);
...
//            outMsg.setContent("感谢您的关注，二维码内容：" + inQrCodeEvent.getEventKey());
            outMsg.setContent("感谢您的关注，二维码内容");
            render(outMsg);	
        }
